...
class EwaldBlock(torch.nn.Module):
    ...
    def __init__(
        self,
        shared_downprojection: DenseLayer,
        emb_size_atom: int,
        downprojection_size: int,
        num_hidden: int,
        activation=None,
    ...
    def get_mlp(self, units_in, units, num_hidden, activation):
        dense1 = DenseLayer(units_in, units, activation=activation, bias=False)
        mlp = [dense1]
        res = [
    ...
    def forward(
        ...
        h: torch.Tensor,
        x: torch.Tensor,
        k: torch.Tensor,#k_grid or k_vectors to calculate dot product of k and x,k^Tx
        num_batch: int,
