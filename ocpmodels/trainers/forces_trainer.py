...
class ForcesTrainer(BaseTrainer):
        ...
        for i, batch_list in tqdm(
            ...
            if per_image:
                ...
                systemids = [
                    str(i) + "_" + str(j)
                    for i, j in zip(
                        batch_list[0].sid, batch_list[0].fid
                    )
                ]
                ...
                if self.ema:
                    self.ema.restore()
                return predictions
        #print(f'=========force:{predictions["forces"]}')
        # with open('/mnt/gs21/scratch/jinhongn/pyg/own_EwaldMP/zinc/predict_force.pkl', 'wb') as f:
        #     pickle.dump(predictions["forces"], f)
        predictions["forces"] = np.array( predictions["forces"], dtype=object) #np.array(predictions["forces"])
    ...
    def train(self, disable_eval_tqdm=False):
        ...
        max_steps = self.config["optim"].get("max_steps")
